 ...
 *
 * Output: test-results/cafe-a-to-z/report.json + summary.md + *.png
 */
const { test, expect } = require('@playwright/test');
const fs = require('fs');
const path = require('path');

const OUT = path.join(__dirname, 'test-results', 'cafe-a-to-z');
const PAGE = '/cafe.html';
const SETTLE = 450;
...
async function snapshot(page) {
  return page.evaluate(() => {
    ...
    return {
      ...
      clientOpen: false, clientHead: null,
      scheduleStatuses: ['colNew', 'colPrep', 'colReady'].map((id) => document.querySelectorAll('#' + id + ' .order').length).join(','),
      queueText: txt(document.getElementById('attn'))?.slice(0, 60),
      availOn: [...document.querySelectorAll('#stockList .av')].map((e) => e.innerText).join(',').slice(0, 60),
      tasksDone: document.querySelectorAll('#histList .hist-line').length,
      toast: [...document.querySelectorAll('.toast')].map((t) => t.innerText.trim()).join(' | ') || null,
      banner: txt(document.getElementById('openChip')),
...
test('cafe A-to-Z audit', async ({ page, context, baseURL }) => {
