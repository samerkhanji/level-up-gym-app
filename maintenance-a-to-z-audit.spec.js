 ...
 *
 * Output: test-results/maintenance-a-to-z/report.json + summary.md + *.png
 */
const { test, expect } = require('@playwright/test');
const fs = require('fs');
const path = require('path');

const OUT = path.join(__dirname, 'test-results', 'maintenance-a-to-z');
...
async function snapshot(page) {
  return page.evaluate(() => {
    ...
    return {
      ...
      liveOpen: document.getElementById('overlay')?.classList.contains('show') || false,
      liveTitle: txt(document.querySelector('#modalBox h3')),
      clientOpen: !document.getElementById('assetCard')?.classList.contains('hidden'),
      clientHead: txt(document.querySelector('#assetCard .eyebrow')),
      scheduleStatuses: ['colSafety', 'colMine', 'colAttn', 'colDone'].map((id) => document.querySelectorAll('#' + id + ' .wo').length).join(','),
      queueText: txt(document.getElementById('colAttn'))?.slice(0, 60),
      availOn: [...document.querySelectorAll('#assetList .st')].map((e) => e.innerText).join(',').slice(0, 80),
      tasksDone: document.querySelectorAll('#partList .partrow').length,
      toast: [...document.querySelectorAll('.toast')].map((t) => t.innerText.trim()).join(' | ') || null,
      banner: txt(document.getElementById('stCrit')),
...
function diff(b, a) {
  ...
  if (a.liveOpen && !b.liveOpen) c.push(`chooser opened: "${(a.liveTitle || '').slice(0, 50)}"`);
  if (a.scheduleStatuses !== b.scheduleStatuses) c.push(`board lanes: ${b.scheduleStatuses} → ${a.scheduleStatuses}`);
  if (a.availOn !== b.availOn) c.push('availability changed');
  if (a.tasksDone !== b.tasksDone) c.push(`history entries: ${b.tasksDone} → ${a.tasksDone}`);
  if (a.banner !== b.banner) c.push(`status chip: ${b.banner} → ${a.banner}`);
...
test('maintenance A-to-Z audit', async ({ page, context, baseURL }) => {
  ...
  await load();
  const SEL = 'button, a[href], .av[data-av]';
  const controls = await page.evaluate((sel) => {
    const vis = (el) => { const r = el.getBoundingClientRect(); return r.width > 0 && r.height > 0; };
    const list = [];
    document.querySelectorAll(sel).forEach((el) => {
      if (!vis(el) || el.closest('#overlay')) return;
