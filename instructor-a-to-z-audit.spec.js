 ...
 *
 * Output: test-results/instructor-a-to-z/report.json + summary.md + *.png
 */
const { test, expect } = require('@playwright/test');
const fs = require('fs');
const path = require('path');

const OUT = path.join(__dirname, 'test-results', 'instructor-a-to-z');
const PAGE = '/instructor.html';
const SETTLE = 450;
...
async function snapshot(page) {
  return page.evaluate(() => {
    ...
    return {
      ...
      clientOpen: !document.getElementById('classCard')?.classList.contains('hidden'),
      clientHead: txt(document.querySelector('#classCard .eyebrow')),
      scheduleStatuses: [...document.querySelectorAll('#classList .st')].map((e) => e.innerText).join(','),
      queueText: txt(document.getElementById('attn'))?.slice(0, 60),
      availOn: txt(document.getElementById('stBooked')),
      tasksDone: document.querySelectorAll('#classList .cls').length,
      toast: [...document.querySelectorAll('.toast')].map((t) => t.innerText.trim()).join(' | ') || null,
      banner: txt(document.getElementById('stWait')),
...
test('instructor A-to-Z audit', async ({ page, context, baseURL }) => {
