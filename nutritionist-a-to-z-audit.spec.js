 ...
 *
 * Output: test-results/nutritionist-a-to-z/report.json + summary.md + *.png
 */
const { test, expect } = require('@playwright/test');
const fs = require('fs');
const path = require('path');

const OUT = path.join(__dirname, 'test-results', 'nutritionist-a-to-z');
const PAGE = '/nutritionist.html';
const SETTLE = 450;
...
async function snapshot(page) {
  return page.evaluate(() => {
    ...
    return {
      ...
      liveOpen: document.getElementById('planCard') ? !document.getElementById('planCard').classList.contains('hidden') : false,
      liveTitle: txt(document.querySelector('#planCard .eyebrow')),
...
function diff(b, a) {
  ...
  if (a.liveOpen && !b.liveOpen) c.push(`plan builder opened: "${(a.liveTitle || '').slice(0, 50)}"`);
...
test('nutritionist A-to-Z audit', async ({ page, context, baseURL }) => {
